...
class _SecuritySettingsScreenState extends State<SecuritySettingsScreen> {
        ...
        children: [
          ...
          _settingCard(
            ...
            trailing: Switch(
              ...
              // Ràng buộc US-14: "Phải xác thực lại danh tính trước khi
              // cho phép thay đổi bất kỳ cài đặt bảo mật nào" — trước đây
              // chỗ này đổi trực tiếp không cần xác thực lại.
              onChanged: (v) => _toggleBiometricWithReauth(context, v),
            ),
          ),
  ...
  /// US-14: bắt xác thực lại (vân tay/mật khẩu) trước khi cho phép bật/tắt
  /// đăng nhập vân tay — thao tác nhạy cảm nhất còn lại trong màn này sau
  /// khi mục Change Password đã được bỏ.
  Future<void> _toggleBiometricWithReauth(BuildContext context, bool newValue) async {
    ...
    // ReauthScreen tự gọi authService.unlock() khi xác thực đúng, nên chỉ
    // cần kiểm tra trạng thái khóa sau khi quay lại: nếu vẫn đang unlock
    // (đã xác thực xong) thì mới thực sự đổi giá trị switch.
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(newValue
              ? 'Đã bật đăng nhập vân tay'
              : 'Đã tắt đăng nhập vân tay'),
        ),
      );
    }
  }
